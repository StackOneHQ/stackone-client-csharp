...
namespace StackOneHQ.Client
{
    ...
    public interface IDocumentsDrives
    {
        /// <summary>
        /// List Drives.
        /// </summary>
        /// <param name="request">A <see cref="DocumentsListDrivesRequest"/> parameter.</param>
        /// <param name="retryConfig">The retry configuration to use for this operation.</param>
        /// <returns>An awaitable task that returns a <see cref="DocumentsListDrivesResponse"/> response envelope when completed.</returns>
        ...
        public  Task<DocumentsListDrivesResponse> ListAsync(
            DocumentsListDrivesRequest request,
            RetryConfig? retryConfig = null
        );
    }

    public class DocumentsDrives: IDocumentsDrives
    {
        /// <summary>
        /// SDK Configuration.
        /// <see cref="SDKConfig"/>
        ...
        public DocumentsDrives(SDKConfig config)
        {
        ...
        /// <exception cref="BadGatewayResponseException">Bad gateway error. Thrown when the API returns a 502 response.</exception>
        /// <exception cref="APIException">Default API Exception. Thrown when the API returns a 4XX or 5XX response.</exception>
        public async  Task<DocumentsListDrivesResponse> ListAsync(
            DocumentsListDrivesRequest request,
            RetryConfig? retryConfig = null
        )
        {
            ...
            var urlString = URLBuilder.Build(baseUrl, "/unified/documents/drives", request, null);
            ...
            var hookCtx = new HookContext(SDKConfiguration, baseUrl, "documents_list_drives", null, SDKConfiguration.SecuritySource);
            ...
            if(responseStatusCode == 200)
            {
                if(Utilities.IsContentTypeMatch("application/json", contentType))
                {
                    ...
                    try
                    {
                        obj = ResponseBodyDeserializer.DeserializeNotNull<DrivesPaginated>(httpResponseBody, NullValueHandling.Ignore);
                    }
                    catch (Exception ex)
                    {
                        throw new ResponseValidationException("Failed to deserialize response body into DrivesPaginated.", httpRequest, httpResponse, httpResponseBody, ex);
                    }
