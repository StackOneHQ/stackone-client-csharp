...
namespace StackOneHQ.Client
{
    ...
    public interface IDrives
    {
        ...
        /// Get Drive.
        /// </summary>
        /// <param name="request">A <see cref="DocumentsGetDriveRequest"/> parameter.</param>
        /// <param name="retryConfig">The retry configuration to use for this operation.</param>
        /// <returns>An awaitable task that returns a <see cref="DocumentsGetDriveResponse"/> response envelope when completed.</returns>
        ...
        public  Task<DocumentsGetDriveResponse> GetAsync(
            DocumentsGetDriveRequest request,
    ...
    public class Drives: IDrives
    {
        ...
        /// <exception cref="BadGatewayResponseException">Bad gateway error. Thrown when the API returns a 502 response.</exception>
        /// <exception cref="APIException">Default API Exception. Thrown when the API returns a 4XX or 5XX response.</exception>
        public async  Task<DocumentsGetDriveResponse> GetAsync(
            DocumentsGetDriveRequest request,
            RetryConfig? retryConfig = null
        )
        {
            ...
            var urlString = URLBuilder.Build(baseUrl, "/unified/documents/drives/{id}", request, null);
            ...
            var hookCtx = new HookContext(SDKConfiguration, baseUrl, "documents_get_drive", null, SDKConfiguration.SecuritySource);
            ...
            if(responseStatusCode == 200)
            {
                if(Utilities.IsContentTypeMatch("application/json", contentType))
                {
                    ...
                    try
                    {
                        obj = ResponseBodyDeserializer.DeserializeNotNull<DriveResult>(httpResponseBody, NullValueHandling.Ignore);
                    }
                    catch (Exception ex)
                    {
                        throw new ResponseValidationException("Failed to deserialize response body into DriveResult.", httpRequest, httpResponse, httpResponseBody, ex);
                    }
